...
var data = ['images/Mett_Pattern_Mett Gradient.png','images/Mett_Pattern_Mett Gradient.png','images/Mett_Pattern_Mett Gradient.png','images/Mett_Pattern_Mett Gradient.png'];
...
/* 手动修改了行列 8行5列 */
bgAnimation({ele: '#bg-show-wrap-2', row: 8, col: 5}, interval(10000));


/* 以下两种方式，不使用 scss 但需要导入 changContain.css */

/* change-contain.css 有一套默认的效果 7行9列 需要将背景变换的父元素加上 .change-contain 类名 */
bgAnimation({ele: '.change-contain', scss: false}, interval(10000));

/* 若不使用行列默认值，5行4列，也不使用 scss ，使用如下方式 */
bgAnimation({ele: '#bg-show-wrap-4', row: 5, col: 4, scss: false}, interval(10000));
